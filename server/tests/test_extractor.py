...
from app.services.extractor import classify_commit_type, extract_git_history, parse_git_path


class TestExtractor(unittest.TestCase):
    def test_classify_commit_type(self):
        self.assertEqual(classify_commit_type("feat(api): add user login endpoint"), "feat")
        self.assertEqual(classify_commit_type("fix: resolve null pointer crash"), "fix")
        self.assertEqual(classify_commit_type("refactor!: rewrite database queries"), "refactor")
        self.assertEqual(classify_commit_type("docs: update README installation guide"), "docs")
        self.assertEqual(classify_commit_type("test: add unit tests for extractor"), "test")
        self.assertEqual(classify_commit_type("chore(deps): bump dependencies"), "chore")
        self.assertEqual(classify_commit_type("Initial commit"), "other")
    ...
    def test_extract_git_history_on_local_repo(self):
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", "feat: initial commit"], cwd=temp_dir, check=True)
            ...
            subprocess.run(["git", "commit", "-m", "refactor: rename app and add utils"], cwd=temp_dir, check=True)

            # Extract history using extractor
            commits, file_diffs, total_commits, total_files, latest_sha = extract_git_history(
            ...
            self.assertIsNotNone(latest_sha)
            self.assertEqual(commits[0]["commit_type"], "refactor")
            self.assertEqual(commits[1]["commit_type"], "feat")
            ...
            # Test incremental sync with since_sha
            first_commit_sha = commits[1]["sha"]
            delta_commits, delta_diffs, delta_c_cnt, delta_f_cnt, new_latest_sha = extract_git_history(
                temp_dir, "test_repo_id", "test_user_id", since_sha=first_commit_sha
            )
            self.assertEqual(len(delta_commits), 1)
            self.assertEqual(delta_commits[0]["commit_type"], "refactor")
